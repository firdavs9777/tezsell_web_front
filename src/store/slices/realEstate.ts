...
import { AgentStatus, BecomeAgentRequest, BecomeAgentResponse, GetAgentsQueryParams, GetPropertiesQueryParams, LocationChoices, MapBounds, MapStatistics, PaginatedResponse, Property, PropertyInquiry, PropertyStats, RealEstateAgent } from "@store/type";
...
export const realEstateApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getProperties: builder.query<PaginatedResponse<Property>, GetPropertiesQueryParams | void>({
      query: (params) => {
    ...
    getPropertiesInMapBounds: builder.query<Property[], { bounds: MapBounds; filters?: GetPropertiesQueryParams }>({
    ...
    // Property inquiries
    createPropertyInquiry: builder.mutation<PropertyInquiry, Omit<PropertyInquiry, 'id' | 'user' | 'is_responded' | 'created_at'>>({
      query: (inquiryData) => ({
    ...
    // Agent endpoints
    getAgents: builder.query<PaginatedResponse<RealEstateAgent>, GetAgentsQueryParams | void>({
      query: (params) => {
        const searchParams = new URLSearchParams();
    ...
    respondToInquiry: builder.mutation<any, { inquiryId: string; response: string }>({
    ...
    updateAgentProfile: builder.mutation<RealEstateAgent, Partial<RealEstateAgent>>({
      query: (profileData) => ({
    ...
    // Admin endpoints
    getPendingAgentApplications: builder.query<PaginatedResponse<RealEstateAgent>, void>({
      query: () => `${PENDING_AGENTS_URL}/`,
      providesTags: ['Agent'],
    }),

    verifyAgent: builder.mutation<any, { agentId: number; approved: boolean; rejection_reason?: string }>({
