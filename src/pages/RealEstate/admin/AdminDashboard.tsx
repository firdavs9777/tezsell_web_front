...
const AdminDashboard: React.FC = () => {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            {t("admin_error_loading_dashboard")}
          </h2>
          <p className="text-gray-600 mb-4">{t("admin_failed_to_load_data")}</p>
          <button
            onClick={() => refetch()}
  ...
  const propertyTypesData = dashboardData.property_stats.property_types.map(
    (item, index) => ({
      name:
  ...
  const citiesData = dashboardData.property_stats.by_city
    .slice(0, 5)
    .map((item) => ({
      name: item.city,
  ...
  const districtsData = dashboardData.property_stats.by_district
    .slice(0, 5)
    .map((item) => ({
      name:
        item.district.length > 15
  ...
  const inquiryTypesData = dashboardData.inquiries.by_type.map(
    (item, index) => ({
      name:
        item.inquiry_type.charAt(0).toUpperCase() + item.inquiry_type.slice(1),
      value: item.count,
      fill: COLORS[index % COLORS.length],
    })
  );
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard
            title={t("admin_dashboard_total_properties")}
            value={dashboardData.overview.total_properties}
            subtitle={`${dashboardData.overview.active_properties} ${t(
              "admin_active"
            )}`}
            icon={Home}
            color="blue"
          />
          <StatCard
            title={t("admin_dashboard_total_agents")}
            value={dashboardData.overview.total_agents}
            subtitle={`${dashboardData.overview.verified_agents} ${t(
              "admin_verified"
            )}`}
            icon={UserCheck}
            color="green"
          ...
          <StatCard
            ...
            subtitle={`${dashboardData.user_activity.active_users_7d} ${t(
              "admin_active_7d"
            )}`}
            icon={Users}
            color="purple"
          />
          <StatCard
            title={t("admin_dashboard_total_views")}
            value={dashboardData.user_activity.total_views}
            subtitle={`${dashboardData.user_activity.views_last_30d} ${t(
              "admin_this_month"
            )}`}
            icon={Eye}
            color="yellow"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard
            title={t("admin_avg_sale_price")}
            value={`$${dashboardData.property_stats.avg_sale_price.toLocaleString()}`}
            subtitle={t("admin_avg_sale_price_subtitle")}
            icon={DollarSign}
            color="green"
          />
          <StatCard
            title={t("admin_total_portfolio_value")}
            value={`$${(
              dashboardData.property_stats.total_property_value / 1000000
            ).toFixed(1)}M`}
            subtitle={t("admin_total_portfolio_value_subtitle")}
            icon={TrendingUp}
            color="blue"
          />
          <StatCard
            title={t("admin_avg_price_per_sqm")}
            value={`$${dashboardData.property_stats.avg_price_per_sqm.toFixed(
              0
            )}`}
            subtitle={t("admin_avg_price_per_sqm_subtitle")}
            icon={Building}
            color="purple"
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("admin_property_types_distribution")}
            </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("admin_properties_by_district")}
            </h3>
            <ResponsiveContainer width="100%" height={300}>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("admin_inquiry_types_distribution")}
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <StatCard
            title={t("admin_agent_verification_rate")}
            value={`${dashboardData.agent_stats.verification_rate}%`}
            subtitle={t("admin_agent_verification_rate_subtitle")}
            icon={UserCheck}
            color="green"
          />
          <StatCard
            title={t("admin_inquiry_response_rate")}
            value={`${dashboardData.inquiries.response_rate}%`}
            subtitle={t("admin_inquiry_response_rate_subtitle")}
            icon={MessageCircle}
            color="blue"
          />
          <StatCard
            title={t("admin_avg_views_per_property")}
            value={dashboardData.engagement.avg_views_per_property}
            subtitle={t("admin_avg_views_per_property_subtitle")}
            icon={Eye}
            color="yellow"
          />
          <StatCard
            title={t("admin_featured_properties")}
            value={dashboardData.overview.featured_properties}
            subtitle={t("admin_featured_properties_subtitle")}
            icon={Star}
            color="purple"
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("admin_most_viewed_properties")}
            </h3>
            <div className="space-y-4">
              {dashboardData.engagement.most_viewed
                .slice(0, 5)
                .map((property, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500">
                        {property.city} • {property.views_count}{" "}
                        {t("admin_views")}
                      </p>
                    </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {t("admin_top_performing_agents")}
            </h3>
            <div className="space-y-4">
              {dashboardData.agent_stats.top_agents
                .slice(0, 5)
                .map((agent, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500">
                        {agent.agency_name} • {agent.total_sales}{" "}
                        {t("admin_sales")}
                      </p>
                    </div>
                    <div className="flex items-center">
            ...
            <div className="flex items-center p-4 bg-red-50 rounded-lg">
              ...
              <div>
                <p className="text-sm font-medium text-red-900">
                  {t("admin_properties_without_images")}
                </p>
            ...
            <div className="flex items-center p-4 bg-blue-50 rounded-lg">
              ...
              <div>
                <p className="text-sm font-medium text-blue-900">
                  {t("admin_pending_agent_verification")}
                </p>
