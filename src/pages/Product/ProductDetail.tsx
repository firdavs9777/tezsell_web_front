  ...
  const navigate = useNavigate();
   useEffect(() => {
    if (singleProduct?.product.images.length > 0) {
      setSelectedImage(`${BASE_URL}/products${singleProduct.product.images[0].image}`);
    }
   }, [singleProduct]);
...
<div className="product-detail-container">
      ...
      <div className="product-detail-right">
        ...
        <p className="product-description">{singleProduct.product.description}</p>
        <div className="product-info">
          <p className="product-price">
            {formatPrice(singleProduct.product.price)} {singleProduct.product.currency}
          </p>
          <p className="product-availability">
            {singleProduct.product.in_stock ? "In Stock" : "Out of Stock"}
          </p>
          <p className="product-condition">Condition: {singleProduct.product.condition}</p>
    ...
    <div key={item.id} className="rec-product-card" onClick={() => redirectHandler(item.id)}>
        <img
       src={`${BASE_URL}/products${item.images[0]?.image}`}
        ...
        <p className="rec-product-description">{item.description.slice(0, 10)}...</p>
